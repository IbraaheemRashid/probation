...
namespace Netcode.Transports.Facepunch
{
    ...
    public class FacepunchTransport : NetworkTransport, IConnectionManager, ISocketManager
    {
        ...
        public override void Initialize(NetworkManager networkManager = null)
        {
            ...
            // PATCHED: only initialise Steam if nothing else already has. The game owns the
            // Steam lifetime (SteamManager), and it has to - lobby creation happens before
            // NetworkManager ever initialises this transport.
        ...
        public override void Shutdown()
        {
            try
            {
                ...
                // PATCHED: do NOT call SteamClient.Shutdown() here. NetworkManager.Shutdown()
                // runs whenever you leave a session, and tearing down the whole Steam client
                // then leaves RunCallbacks pumping a dead client, kills the overlay, and makes
                // every subsequent host attempt fail. SteamManager owns shutdown.
            }
            catch (Exception e)
            {
                if (LogLevel <= LogLevel.Error)
